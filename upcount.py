...
file_name = f'number-{datetime.datetime.now().strftime("%Y%m%d%H%M")}.txt'
...
#####################################################
    ...
    file_name = f'number-{today.strftime("%Y%m%d%H%M")}.txt'
    file_path = os.path.join(output_dir, file_name)
    if os.path.exists(file_path):
        with open(file_path, 'r') as file:
            last_line = file.readlines()[-1]
            last_timestamp, frame_idx, last_total_num = last_line.split('\t')
